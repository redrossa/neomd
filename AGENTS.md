...
- **Approved #43 app-security exception:** NeoMD's app Debug and Release configurations intentionally disable App Sandbox (`ENABLE_APP_SANDBOX = NO`) for direct local reading; this supersedes the earlier sandbox-preservation/session-folder-grant policy, not the read-only application contract. Remove the app-managed enclosing-folder permission flow, including image Allow-folder actions. Do not add remembered folder grants, security-scoped bookmark storage, folder setup/management, or helper escapes. Unsandboxed NeoMD is not compatible with Mac App Store distribution. Continue handling macOS privacy/TCC, POSIX/ACL and volume failures with actionable feedback; sandbox removal does not grant unrestricted access. Never modify source bytes/mtime, execute arbitrary files, or weaken other security controls. Apps/executables remain reveal-only in Finder; other safe file types use explicit system opening. This narrowly approved exception does not authorize changes to signing/team, bundle identifiers, deployment targets, hardened runtime, ATS or build-script sandboxing, and is not permission to change security settings merely to make tests pass.
