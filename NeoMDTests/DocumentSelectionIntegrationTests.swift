...
@MainActor struct DocumentSelectionIntegrationTests {
    ...
    @Test func focusedLinkAttributesAndUnavailableImageProjectionRemainIndependent() throws {
        ...
        var alternative = AttributedString("alt")
        alternative.markdownImage = MarkdownImage(source: nil, lightSource: nil, darkSource: nil)
        let document = MarkdownRenderDocument(nodes: [.init(id: 0, kind: .paragraph, text: alternative)], rootIDs: [0])
        let nativeValue = MarkdownLinkedImageContent.project(.init(text: alternative, states: [:], dark: false, width: 300, headingLevel: nil))
