...
**Current authority — M1-DAG-001:** #15 is completed via PR #54. The user replaced sequential scheduling with the adopted [dependency plan](milestone-1-dependency-plan.md) for #16–24, subject to triage and documented holds; #14 remains deferred/open. This supersedes the historical status paragraph below only where it specifies next/sequential work. No reviewer or interaction tests; native criteria and milestone acceptance remain user-owned. Historical decisions and evidence remain intact.

Status: Historical brainstorm; unresolved proposals remain draft. Published GitHub issues are the implementation contracts. Stories #1–13 and priority stories #40–43, #48, #50 and #52 are completed. The [priority pivot](milestone-1-priority-pivot.md), [focus-outline pivot](milestone-1-focus-outline-pivot.md), [Command-click pivot](milestone-1-command-click-pivot.md) and [window-cascade pivot](milestone-1-window-cascade-pivot.md) retain their approved scope and evidence limits. #50 restores Command-click local Markdown links to an additional reader, including already-open destinations; ordinary clicks still replace the source. #52 adds source-relative placement; its native/physical-display interaction checks remain deferred/unrun, not passed. **The user renewed sequential #15–24 implementation authorization, explicitly skipping #14, which remains entirely deferred/open. #15 is next.** The [canonical milestone order](https://github.com/redrossa/neomd/milestone/1) and [#15 table clarification](https://github.com/redrossa/neomd/issues/15#issuecomment-5628747670) supersede prior pauses and hidden-YAML proposals. [D1 dependency approval](https://github.com/redrossa/neomd/issues/15#issuecomment-5628830891) and [D2 user boundary/recovery approval](https://github.com/redrossa/neomd/issues/15#issuecomment-5628889673) resolve triage blockers; see [accepted content plan](m1-15-content-plan.md) and [validation map](m1-15-validation.md). Triage readiness is not implementation or behavior acceptance. Gates remain Debug build and relevant NON-INTERACTION units only, with no reviewer; no UI/E2E/manual scripted interaction testing or prototypes. All native/visual/AX interaction coverage remains DEFERRED/UNRUN. #48's quiet enclosing-focus amendment, #36 full-reflow failure/defer and #38 cue C5 unchecked/deferred disposition remain unchanged. Combined milestone acceptance is user-owned. Historical implementation/status paragraphs below retain their original planning/evidence meaning, superseded only where this current authority explicitly says so; they are not permission to run old commands, resume #14 or claim validation.
...
**Current disposition: entirely user-deferred/open; explicitly skipped.** The historical proposal below is not authorized for implementation. The user has separately authorized sequential M1-15 through M1-24; skipping #14 does not resolve its product proposals or close its issue.
...
Approved [#15](https://github.com/redrossa/neomd/issues/15) acceptance criteria (all implementation/validation pending at triage):
...
- Render document-leading YAML front matter as a readable metadata table, like GitHub, rather than hiding it or displaying raw YAML delimiters and source.

[Table approval](https://github.com/redrossa/neomd/issues/15#issuecomment-5628747670) supersedes hidden YAML. [D1](https://github.com/redrossa/neomd/issues/15#issuecomment-5628830891) approves offline pinned MIT libyaml 0.2.5 (`2c891fc7a770e8ba2fec34fc6b545c672beb37e6`) via bounded events into flat owned data, no object constructors/custom-tag execution. [D2](https://github.com/redrossa/neomd/issues/15#issuecomment-5628889673) approves closed document-leading YAML precedence for mappings, nested sequences, scalars and empty data: optional initial BOM; column-zero opening `---` and first closing `---` or `...`, optional trailing ASCII spaces/tabs; LF/CRLF/CR. No leading blank/preamble or later/code/container detection. Unclosed input remains ordinary Markdown. Closed malformed/over-limit candidates retain full literal content plus quiet explanation before the normally rendered body; recovery text must not execute Markdown/HTML/link/image instructions. Scalar may use a Value row; empty metadata has an explicit empty state. No independent TOML/JSON front-matter format or general #19 Markdown table scope is added. [Plan](m1-15-content-plan.md), [inert fixture packet](fixtures/m1-15-content/README-fixture.md) and [validation map](m1-15-validation.md) separate planned oracles from actual worker evidence; all native/visual/AX interactions remain deferred/unrun.
...
2. Historical separate-window/session-folder-access proposals were superseded by the explicit #43 approvals in the priority pivot: active-window replacements, Markdown-only native reading and unsandboxed read-only access without app-managed grants. Command-N's single-file picker and one-file-at-a-time batch defaults were approved and implemented in #43. Approved #50 now supersedes only the Command-N-only restriction, restoring Command-click local Markdown links to an additional reader. Current-folder leading-slash paths remain unchanged.
