...
## M1-P9 / #65 — upward trackpad momentum stalls

[Issue #65](https://github.com/redrossa/neomd/issues/65) · [validation and non-interaction allowlist](m1-p9-validation.md) · [inert mixed-height fixture / policy oracles](fixtures/m1-p9-scroll/README-fixture.md).

Triage **ACCEPTED bounded implementation readiness**, refreshed exact base `445ed2b897934d339eb01b750d37b03edf392dce` after #64 / PR #66, verified `openai-codex/gpt-6-astra/high`. Source confirms passive scroll metrics/frame/anchor SwiftUI-state writes beside lazy measurement and a missing user-scroll guard on automatic viewport restoration/existing-find reveal. The 150 ms hold is Page Up/Down only; ordinary capture/history/selection does not directly request a scroll. SwiftUI's lazy estimate correction remains a plausible contribution, not a proven native clamp. No original failing file, native reproduction or warning trace was available. Command-Up/Home remains untried.

Bounded repair: separate latest passive observations from command/view state and prevent automatic old-anchor/find correction during user tracking/interacting/decelerating, including stale delayed restoration and detached-find continuations after idle. Preserve explicit commands, presentation guards and #64 owner-scoped selection busy/lifetime/activity; scroll-idle must not release selection or queued size/refresh prematurely. Native reader/size/history code is unchanged by PR #66; do not rewrite its new selection implementation. Keep lazy roots, native selection, actual measurement, initial margins, history semantics, local overflow and source immutability. No eager-stack conversion, guessed fixed heights, general selection rewrite or repaired-hang claim. Pure policy regressions must be wired to the actual callbacks, not a disconnected event/physics substitute.

Worker build/unit evidence: **UNRUN at triage**. Record exact tested head, commands, nonzero selected-suite counts/failures/skips and artifact corrections in `m1-p9-validation.md`. Native momentum feel, warning reproduction, keyboard/focus/selection/AX/appearance/resize and combined behavior remain **DEFERRED/UNRUN**. Unit success cannot establish #65 fixed; obtain the user's native outcome before claiming resolution. If still reproducible, return evidence for refreshed diagnosis rather than widening scope silently. #14/#36/#38 remain deferred/open; layout/selection-area overlap is not proven shared causality, and no deferral is extended. Preserve every historical entry and failure; final native testing and milestone acceptance remain user-owned.

### P8/P9 authorized bounded final-validation addendum

The coordinator's later seven-method P8/P9 exception supersedes the historical no-interaction statements only for `PrioritySelectionUITests`' five targeted methods and `PriorityScrollUITests`' two generic-scroll methods. [Final methodology and evidence](m1-p9-validation.md#final-worker-freeze-and-bounded-evidence) preserves exact selectors, 51-unit allowlist, clean-head freeze protocol and remaining limitations. Pre-freeze bounded development runs passed 7 UI + 51 units; final clean-head gates are **pending at documentation freeze**, with actual SHA/results subsequently recorded in PR #67 and the external final-head handoff, not inferred here.

Additional triage fixtures are `fixtures/m1-priority-e2e/selection-links.md` (unchanged selection prefix plus inert trailing extent) and `priority-scroll-navigation.md` / `priority-scroll-navigation-recipe.json` (eight original mixed-height copies, internal AMBER link, unique fifth-copy sentinel). Original fixtures/oracles remain unchanged. See [link fixture correction](m1-priority-link-fixture-repair.md) and [P9 setup amendment](m1-p9-internal-navigation-setup.md). Adopted readiness, virtual-table ancestry, owned companion placement, pre-click geometry transfer and bounded 250 ms completion corrections do not relax measured selection/landing/scroll criteria. Source Command-click preservation and key-window selection redraw repairs are documented separately from setup fixes. Exact signatures/entitlements, binary/PID/window identity, source bytes/mtime through owned Quit and no survivors are final gates. Historical failed runs remain failures. Synthetic generic scrolling is not physical momentum; warm retained-layout repetition, physical-trackpad feel, other native/manual/AX coverage, #14/#20/#36/#38 and milestone acceptance remain deferred/user-owned. No broad catalog replacement or other story acceptance is implied.
...
## M1-24 / #24 — recent files and durable reading position

[Issue #24](https://github.com/redrossa/neomd/issues/24). Exact triage base `acc932318bda21a9cafde3e2ea53d612d1bcbe5a`; verified Astra/high. **ACCEPTED for implementation readiness, not completed or native-validated.** [Validation map and inspected unit allowlist](m1-24-validation.md), [inert history packet](fixtures/m1-24-history/README-fixture.md).

Retain native Open Recent and Clear Menu; add bounded versioned private position history keyed by complete normalized file URL. Restore only successful eligible reopen presentations, preserving explicit fragments and each existing reader's live place. Content/context/fraction mapping, not persisted integer IDs, handles inserted metadata, duplicates and missing target content. Original capture order—not last close order—chooses among independent readers. Missing paths retain the other readable document and show recovery feedback. No automatic reopen-all-windows, source sidecars/writes, folder bookmarks/grants or observer implementation.

#23 owns shared live snapshot/locator and first contended capture/apply/session/coordinator lease; #24 owns portable history/storage and reopen restoration. #19 owns table/frame/bridge/traversal seams; reconcile actual merged APIs without unmerged dependencies. The unique catalog entry is appended after coordinator hash reconciliation, never as a stale full-file replacement.

Worker Debug build and inspected non-interaction unit evidence: **UNRUN at triage**, to be recorded in `m1-24-validation.md` with exact head, selectors, nonzero counts, failures/skips and external logs. All native menus/clear/reopen/relaunch/quit/scroll/AX and interacted source-byte/mtime behavior remains **DEFERRED/UNRUN**. No reviewer or UI/E2E/manual scripted/native-host substitute tests. #14/#36/#38 and their existing failures/deferrals remain unchanged, with no repair or extended waiver. User retains combined milestone acceptance.

Worker evidence: implemented and validated on integration base `5c1940c515533d093b53ab2124a7c06fb84f2994` (canonical `main` after #23). Debug build succeeded; one selector-scoped unit run passed **66/66 cases, 0 failed, 0 skipped** across 11 suites, covering the three new history suites, the allowlisted existing selectors and the four merged #23 refresh/locator suites as integration regression. #24 adds **no second remapper**: `ReadingHistoryLocator` is the durable projection of the merged `DocumentContentLocator`, and `DocumentReaderView.swift` is unmodified. One staged oracle, `deleted-target-neighbor-tie`, diverges from merged #23 neighbour tie-break behavior; the fixture bytes are unchanged and the divergence is recorded for coordinator reconciliation in [m1-24-validation.md](m1-24-validation.md). No UI/E2E/manual/event/host test was run, so every native outcome above stays deferred.

## M1-17 / #17 — recover from file problems

[Issue #17](https://github.com/redrossa/neomd/issues/17) · [encoding approval](https://github.com/redrossa/neomd/issues/17#issuecomment-5642959135) · [criterion/validation map](m1-17-validation.md) · [inert recovery packet](fixtures/m1-17-recovery/README-fixture.md).

Final triage **ACCEPTED for narrow implementation readiness** at `c672c090b6c47a7c8247781af9cb4c3ec6b5dd54`, runtime `openai-codex/gpt-6-astra` high verified. C1/C2/C4/C5 have **ALREADY_SATISFIED source/model implementations**, requiring focused regression coverage, not native checkbox acceptance. C3 remains a **GAP**: preserve actual read/decode causes and attempted path through open/picker/app/local-link routes and present concise actionable native errors on the captured reader rather than a transient notice. Keep permissive BOM / UTF-8 / inferred / Latin-1 decoding; the old encoding hold is resolved. Invalid/truncated UTF-8 and binary fixtures are compatibility inputs, not invented rejection examples. #23 quiet retrying refresh and #24 history/last-good state are merged foundations, not duplicate scope.

Packet contains exact byte-preserved empty/BOM/whitespace/Unicode-mixed-ending/invalid/truncated/binary fixtures, bounded malformed nesting and a directory named .md. cases.json records byte SHA-256 and inert decode/error/preservation/immutability oracles. Triage ran no build, test, prototype or interaction. Worker evidence is **UNRUN / not supplied at triage**; record exact-head Debug build and inspected non-interaction selectors/counts/results/corrections in the validation map. Never run blanket NeoMDTests or a native dialog/window/event substitute.

All native alerts/dialogs/window ownership, actual empty/emoji/appearance/AX/focus, Finder/picker/Recent/link/refresh interactions and repeated native close/no-save/bytes/mtime checks remain **DEFERRED/UNRUN**, not passed by models. #21's leased session/window/menu seams and #16 reader/AX seams require coordinator reconciliation if touched. Preserve #14/#36/#38 deferrals and every prior catalog entry. Final native/business acceptance remains user-owned.
